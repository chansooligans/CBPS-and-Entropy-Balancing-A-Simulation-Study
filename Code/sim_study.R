...
# Models
model_names = c("Baseline Logistic", 
...
dgp = function(n) {
  ...
  X_[[6]] = 0.5 * exp(z1/2) + z2/(1 + exp(z1)) + -0.2 * z3^2 + z1 * z4 - 0.4 * sqrt(z5 - z6) - 0.2 * (z1 + 1.2 * z6)^2 + 0.5 * z7
  ...
  X_[[8]] = 0.5 * exp(z1/2) + z2/(1 + exp(z1)) - 0.2 * z3^2 + z1 * z4 - 0.4 * sqrt(z5 - z6) - 0.2 * (z1 + 1.2 * z6)^2 + 0.5 * z7
...
sim_dgp = function(Z_ = Z_[[1]], X_ = X_[[1]], sim_no = 1, n, B_ = c(0.4, 0.5, 0.3, 0.9), plot = TRUE, nlo = FALSE, dgp_name = "dgp name here") {
  ...
  # Generate treatment vector treat = as.integer(e_Z>runif(n,0,1))
...
ci_results = matrix(nrow = n_sims, ncol = length(model_names), dimnames = list(1:n_sims, model_names))

lin_results = matrix(nrow = n_sims, ncol = length(model_names), dimnames = list(1:n_sims, model_names))
...
while (i <= n_sims) {
  ...
  mod_match = matchit(formula, data = df.temp, method = "nearest", distance = "logit", replace = TRUE)
  df.temp$wt = mod_match$weights
  
  # Mahalanobis Distance Matching
  mod_mahalo = matchit(formula, data = df.temp, method = "nearest", distance = "mahalanobis", replace = TRUE)
  df.temp$wt_mh = mod_mahalo$weights
  
  # CBPS Weights (OVER)
  cbps.over = weightit(formula, data = df.temp, method = "cbps", estimand = "ATT", over = TRUE)
  df.temp$wt.cbps_over = cbps.over$weights
  
  # CBPS Weights (JUST)
  cbps.just = weightit(formula, data = df.temp, method = "cbps", estimand = "ATT", over = FALSE)
  df.temp$wt.cbps_just = cbps.just$weights
  
  # Entropy Balancing Weights 1
  r1 = NULL
  test1 = try(r1 <- ebalance(Treatment = df.temp$treat, X = df.temp[, confounders[[k]]]))
  ...
  r2 = NULL
  test2 = try(r2 <- ebalance(Treatment = df.temp$treat, X = cbind(df.temp[, confounders[[k]]], df.temp[, confounders[[k]]]^2)))
  if (class(test2) %in% "try-error") {
    next
  } else {
    eb.out2 = r2
  }
  ...
  for (j in 1:length(wts)) {
    lm.mod = lm(y ~ ., data = df.temp[, c("y", "treat", confounders[[k]])], weights = df.temp[, wts[j]])
  ...
  if (i%%5 == 0) 
    print(apply(ci_results[1:i, ], 2, mean))
  i = i + 1
}
